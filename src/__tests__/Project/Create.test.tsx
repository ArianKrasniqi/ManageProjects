...
describe("Test <Create />", () => {
  ...
  test("should add user when form is submitted", async () => {
    const { getByTestId,getByLabelText, debug, queryByTestId, container } = render(<Create />, {
      initialState: {
        users: [
          {
            id: 1,
    ...
    fireEvent.change(screen.getByTestId("project-name"), {target: {
      value: "Testing Project Name..."
    }});
    
    fireEvent.change(screen.getByTestId("project-description"), {target: {
